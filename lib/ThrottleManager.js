...
class ThrottleManager {
    ...
    constructor() {
        try {
                ...
                try {
                    ...
                    function ThrottleResponseCB(error, response, body) {
                        ...
                        if (response.statusCode === 200) {
                            request.log(`Successful API CALL: X-ME:[${response.headers['x-me']}] , X-Vidiun-Session:[${response.headers['x-vidiun-session']}] `);
                            request.log(`Got response from API server - body [${body}]`);
                            if (request.callback)
                                request.callback(response);
                        }
                        else {
                            request.error(`Failed API CALL: X-ME:[${response.headers['x-me']}] , X-Vidiun-Session:[${response.headers['x-vidiun-session']}]\n\r ${util.inspect(error)}`);
                            if (request.callback)
                                request.callback(response,error);
                        }
                    }
            ...
            VidiunLogger.error("Error Throttling messages - " + util.inspect(err));
        }
    }
    ...
    resetRequestHeaders(request) {
        ...
        this.generateVidiunRemoteAddr(request);
    }
    ...
    generateVidiunRemoteAddr(request) {
        if(request.remoteIp) {
            ...
            request.headers['X-VIDIUN-REMOTE-ADDR'] = ipHeader;
        }
    }
